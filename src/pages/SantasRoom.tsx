...
const SantasRoom: React.FC<SantasRoomProps> = ({ onNavigateBack, onAddWish }) => {
  // --- Refs ---
  ...
  // --- State ---
  const [hitboxes, setHitboxes] = useState<React.CSSProperties[]>([]);
  ...
  const [cakeInitialStyle, setCakeInitialStyle] = useState<React.CSSProperties>({});
  ...
  const updateElementPositions = useCallback(() => {
    ...
    setBackButtonStyle({
      position: 'absolute', top: `${videoRect.top + 16}px`, left: `${videoRect.left + 56}px`, cursor: 'pointer',
    });

    const scaleX = videoRect.width / video.videoWidth;
    const scaleY = videoRect.height / video.videoHeight;
    const newHitboxes = objectCoordinates.map(obj => ({
        position: 'absolute',
  ...
  useEffect(() => { return () => { turntableAudioRef.current?.pause(); }; }, []);
  ...
  const handleHitboxClick = (objectName: string, index: number) => {
    ...
    if (objectName === 'Turntable') {
      ...
      const cakeHitboxStyle = hitboxes[index];
      setCakeInitialStyle({ top: cakeHitboxStyle.top, left: cakeHitboxStyle.left, width: cakeHitboxStyle.width, height: cakeHitboxStyle.height });
      setIsFadingOut(false);
      setCakeState('zooming');
    } else if (objectName === 'Postcard') {
  ...
  return (
    <div className="relative w-screen h-screen bg-black flex items-center justify-center overflow-hidden">
      ...
      <img src={backButtonImg} alt="Back" style={backButtonStyle} onClick={onNavigateBack} className="absolute z-40 w-48 h-auto p-2 cursor-pointer"/>
      
      <video ref={videoRef} src={roomVideo} autoPlay muted playsInline onTimeUpdate={handleTimeUpdate} className="w-full h-full object-contain" />
      
      {(cakeState === 'hidden' && !showPostcard) && hitboxes.map((style, index) => (
        <div key={index} style={style} onClick={() => handleHitboxClick(objectCoordinates[index].name, index)} className="z-20" />
      ))}

      {cakeState !== 'hidden' && (
        <div className="absolute inset-0 bg-black bg-opacity-50 z-30 flex items-center justify-center" onClick={handleCakeClick}>
            <img
                src={cakeState === 'blown_out' ? cakeImg2 : cakeImg1}
                alt="Christmas Cake"
                style={cakeState === 'zooming' ? cakeInitialStyle : {}}
                className={`cake-image ${cakeState !== 'hidden' ? 'zoomed' : ''} ${isFadingOut ? 'opacity-0' : 'opacity-100'}`}
            />
        </div>
      )}

      {showPostcard && (
        <div className="absolute inset-0 bg-black bg-opacity-50 z-30 flex items-center justify-center animate-fade-in" onClick={() => setShowPostcard(false)}>
          <div className="flex flex-col items-center" onClick={e => e.stopPropagation()}>
            <div className="relative w-[90vw] max-w-lg">
              ...
              <textarea
                            ...
                            className="absolute top-[20%] bottom-[15%] left-[10%] right-[10%] w-[80%] h-[65%] bg-transparent border-none outline-none resize-none p-2 text-gray-800 text-lg placeholder:text-gray-600 text-center"
                          />            </div>
            <button
